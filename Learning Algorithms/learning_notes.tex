...
\textit{E-step: compute the posterior $q(\mathbf{z}_i) = p(\mathbf{z}_i\mid\mathbf{x}_i,\boldsymbol{\theta})$.} Because both distributions above are Gaussian and linearly related, this posterior is available in closed form by completing the square:
\[
...
\textit{M-step: maximize the expected complete-data log-likelihood.} It is convenient to estimate $\mathbf{W}$ and $\boldsymbol{\mu}$ jointly by defining
\[
...
\]
This absorbs the intercept $\boldsymbol{\mu}$ into a single matrix product, the same trick used for bias terms in linear regression.

The M-step requires not just $\mathbb{E}[\tilde{\mathbf{z}}_i\mid\mathbf{x}_i]$ but also $\mathbb{E}[\tilde{\mathbf{z}}_i\tilde{\mathbf{z}}_i^T\mid\mathbf{x}_i]$, because the expected complete-data log-likelihood contains a term quadratic in $\mathbf{z}_i$ (from $\mathbf{z}_i^T\mathbf{W}^T\boldsymbol{\Psi}^{-1}\mathbf{W}\mathbf{z}_i$). Define:
\[
\mathbf{b}_i \triangleq \mathbb{E}[\tilde{\mathbf{z}}_i\mid\mathbf{x}_i] = [\mathbf{m}_i;1]
\]
\[
\mathbf{C}_i \triangleq \mathbb{E}[\tilde{\mathbf{z}}_i\tilde{\mathbf{z}}_i^T\mid\mathbf{x}_i] = \begin{pmatrix}\mathbb{E}[\mathbf{z}_i\mathbf{z}_i^T\mid\mathbf{x}_i] & \mathbb{E}[\mathbf{z}_i\mid\mathbf{x}_i] \\ \mathbb{E}[\mathbf{z}_i\mid\mathbf{x}_i]^T & 1\end{pmatrix}, \qquad \mathbb{E}[\mathbf{z}_i\mathbf{z}_i^T\mid\mathbf{x}_i] = \boldsymbol{\Sigma}_i + \mathbf{m}_i\mathbf{m}_i^T
\]
